...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'startDate' | 'endDate'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      startDate: product.startDate?.format(DATE_FORMAT) ?? null,
      endDate: product.endDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      startDate: restProduct.startDate ? dayjs(restProduct.startDate) : undefined,
      endDate: restProduct.endDate ? dayjs(restProduct.endDate) : undefined,
    };
  }
