...
type RestOf<T extends IMember | NewMember> = Omit<T, 'birthDate' | 'signupDate'> & {
...
export class MemberService {
  ...
  protected convertDateFromClient<T extends IMember | NewMember | PartialUpdateMember>(member: T): RestOf<T> {
    return {
      ...member,
      birthDate: member.birthDate?.format(DATE_FORMAT) ?? null,
      signupDate: member.signupDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMember: RestMember): IMember {
    return {
      ...restMember,
      birthDate: restMember.birthDate ? dayjs(restMember.birthDate) : undefined,
      signupDate: restMember.signupDate ? dayjs(restMember.signupDate) : undefined,
    };
  }
